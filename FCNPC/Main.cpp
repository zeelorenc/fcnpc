...
PLUGIN_EXPORT bool PLUGIN_CALL Load(void **ppData) 
{
	...
	if (logprintf == (logprintf_t)CAddress::FUNC_Logprintf_03ZR4)
	{
		sampVersion = SAMP_VERSION_03Z_R4;
		logprintf("[FCNPC] SA-MP server version 0.3z R4 (" OS_NAME ") detected. Initializing...");
	}

	// Initialize Plugin
	if (sampVersion != SAMP_VERSION_UNKNOWN)
	{
		// Create the server instance
		pServer = new CServer();
		if (!pServer)
		{
			logprintf("[FCNPC] Failed. (Couldn't create server instance)");
			return false;
		}
	}
	else
	{
		logprintf("[FCNPC] Failed. (" OS_NAME " server not supported yet)");
		return true;
	}
	...
	logprintf("");
	logprintf("- Author(s): OrMisicL, Emci_Funtik, Lorenc_, MyU");
